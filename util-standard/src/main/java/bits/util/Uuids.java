...
/**
 * Provides additionaly functionality missing to {@link java.util.UUID},
 * including full implementations of RFC-4122, Variant 4 (Leach-Salz),
 * {@link #createTimeBased() Versions 1 (time-based)},
 * {@link #hashMd5(UUID,String) Version 3 (MD5 name-based, for legacy purposes)},
 * {@link #createRandom() Version 4 (random)},
 * {@link #hashSha1(UUID,String) Version 5 (SHA-1 name-based)}
 * {@link #hashSha256(UUID,String) Custom Version 13 (SHA-256 name-based)}
 *
 ...
 * 0x2 : DCE Security, with embedded POSIX UUID (not supported)
 ...
 * this class have {@code reserved == 0x2}.
 *
...
public final class Uuids {
    ...
    /**
     * Please use {@link #hashSha1} instead. This method uses an older and less
     * secure hash.
     *
     ...
     * @deprecated Insecure. Use {@link #hashSha256}
     */
    public static synchronized UUID hashMd5( UUID optNamespace, String name ) {
        if( sDigestMd5 == null ) {
            sDigestMd5 = createDigest( "MD5" );
        }
        return hash( optNamespace, name, sDigestMd5 );
    }
     ...
     * @deprecated Insecure. Use {@link #hashSha256}
     */
    public static synchronized UUID hashSha1( UUID optNamespace, String name ) {
        if( sDigestSha1 == null ) {
            sDigestSha1 = createDigest( "SHA-1" );
        }
        return hash( optNamespace, name, sDigestSha1 );
    }

    /**
     * Returns a new name-based UUID using SHA-256 hash (version 0xD). This
     * UUID is determined entirely by provided input, and different machines at
     * different times will create the same name-based UUID by providing
     * the same namespace and name. The returned UUID will contain:
     *
     ...
     *
     * hash0:hash1:hash2 - 122 name hash bits. The hash bits are generated
     * by feeding the 16 bytes provided by the namespace UUID into a SHA-256 digest,
     * followed by the UTF-8 bytes of the provided name.
     * <p>
     * reserved - 2 variant bits, set as b10.
     * <p>
     * version - 4 version bits, set as b1101.
     *
     * @param optNamespace The namespace being used. May be {@code null}.
     * @param name Name to hash.
     * @return a new name-based UUID.
     */
    public static synchronized UUID hashSha256( UUID optNamespace, String name ) {
        if( sDigestSha2 == null ) {
            sDigestSha2 = createDigest( "SHA-256" );
        }
        return hash( optNamespace, name, sDigestSha2 );
    }
    ...
    /**
     * Converts UUID into Big-Endian byte stream.
     * 
     * @param u     Input UUID
     * @param out16 Array to receive 16-byte output.    
     * @param off   Offset into output array.
     */
    public static void toBytes( UUID u, byte[] out16, int off ) {
        long mostSig = u.getMostSignificantBits();
        long leastSig = u.getLeastSignificantBits();

        for( int i = 0; i  < 8; i++ ) {
            out16[off+i] = (byte)( mostSig >> 56 - i * 8 );
        }
        off += 8;
        for( int i = 0; i < 8; i++ ) {
            out16[off+i] = (byte)( leastSig >> 56 - i * 8 );
        }
    }
    ...
    private static MessageDigest sDigestSha2    = null;
    ...
    private static UUID hash( UUID optNamespace, String name, MessageDigest digest ) {
        digest.reset();
        ...
        byte[] bytes = digest.digest( name.getBytes( UTF_8 ) );
    ...
    private static MessageDigest createDigest( String name ) {
        try {
            MessageDigest ret = MessageDigest.getInstance( name );
            if( ret.getDigestLength() < 16 ) {
                throw new UnsupportedOperationException( "Insufficient digest length" );
            }
            return ret;
        } catch( Exception ex ) {
            throw new UnsupportedOperationException( "Suitable " + name + " digest provider not found." );
        }
    }
    ...
    private static long fetchClockSequence( SecureRandom rand ) {
        long ret;
        Preferences pref = Preferences.userNodeForPackage( Uuids.class );

        try {
            pref.sync();
            ret = (pref.getInt( "clock_sequence", rand.nextInt() ) + 1) & 0x3FFF;
            pref.putInt( "clock_sequence", (int)ret );
            pref.flush();
        } catch( BackingStoreException ex ) {
            ret = rand.nextInt() & 0x3FFF;
        }
    ...
    private static void digestNodeId( SecureRandom rand, MessageDigest out ) {
        try {
            NetworkInterface net = NetworkInterface.getByIndex( 0 );
            byte[] hardAddress = null;
            if( net != null ) {
                hardAddress = net.getHardwareAddress();
            }
            if( hardAddress != null ) {
                out.update( net.getHardwareAddress() );
            }
